# run this to produce a trained tokenizer:
# load corpus -> train BPE -> build vocab -> report lengths -> save
...
def estimate_token_length(
    ...
    # percentiles here are what context_len gets picked from
...
def train_tokenizer(cfg: LLMConfig = default_cfg) -> Tokenizer:
    ...
    # cap the training slice — BPE cost grows with unique pre-tokens
    max_chars = 5_000_000
    sampled_lines = []
    total = 0
    ...
    # in-memory only; edit config.py to persist the real vocab size
